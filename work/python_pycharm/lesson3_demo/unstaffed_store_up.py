...
def warehouse(item_type):
    ...
    if item_type =="xc":
        return  xc_list
#检测货架上的商品是否需要补货
def check_add_racl(rack,item_type,item_counts):
    if len(rack) == 0:
        print("——正在更新货架，请稍等——")
...
def shopping_house():
    ...
    xc_rack = []
    #货架摆放商品数量
    pm_rack_counts = 1  # 每个货架上摆多少东西
    zc_rack_counts = 1  # 每个货架上摆多少东西
    xc_rack_counts = 1  # 每个货架上摆多少东西
    buy_car =[]
    if_new_user =True
    while True:
        #自动检测货架是否需要补货
        check_add_racl(pm_rack,"pm",pm_rack_counts)
        check_add_racl(zc_rack,"zc",zc_rack_counts)
        check_add_racl(xc_rack,"xc",xc_rack_counts)
        if if_new_user:
            print("欢迎光临")
