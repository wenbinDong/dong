...
__IO uint32_t ADC_ConvertedValueTen[FRE_NUM];
/**
  * @brief  ADC GPIO 初始化
  * @param  无
  * @retval 无
...
void ADC_test_ten(void)
{
	...
	for (i = 0;i<10; i++)
	{
		...
		ADC_ConvertedValueTen[i] =ADC_ConvertedValue[i];	
	}
	for(i=0;i<10;i++)
	{
		value1+=ADC_ConvertedValueTen[i];
	}
	BAT1_ADC_Val1_ten=(uint32_t)((float)value1/ntimes1);
	//printf("\r\n 100us后的ADC数值为 %d \r\n", BAT1_ADC_Val1_ten);
	//printf("\r\n 100us后的ADC数值为 %d \r\n", value1);
