...
/**
  ...
  * @retval 无
  */
static void ADCx_Mode_Config(uint8_t cycle)
{
  ...
  // DMA_InitStructure.DMA_MemoryBaseAddr = (u32)&ADC_ConvertedValue;
	...
	ADC_RegularChannelConfig(macADCx, macADC_CHANNEL, 1, cycle);
...
//	ADC_RegularChannelConfig(ADC1, ADC_Channel_10, 1, cycle);
//	
//	/* Enable ADC1 DMA */
...
{
	ADCx_GPIO_Config();
	ADCx_Mode_Config(macADC_SAMPLETIME_L);
}

void ADCx_Init_H(void)
{
	ADCx_GPIO_Config();
	ADCx_Mode_Config(macADC_SAMPLETIME_H);
}
...
float ADC_test_now()
{
	...
	//这里放控制时序，第一次电压测试（电池放电前电压）
	BAT1_ADC_Valt =ADC_ConvertedValue[0];
