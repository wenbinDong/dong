...
__IO uint32_t i=0;
__IO uint32_t i_nvic=0,i_avg=0,j=0,kkk=0,flag_back=0;
  ...
  if (ADC_GetITStatus(ADC1,ADC_IT_EOC)==SET) 
		{
			...
			if(i_nvic==3)
			{
				...
				else if(flag_nvic==1)
				{
					...
					ADC_ConvertedValue[i_avg++]=ADC_ConvertedValueNVIC[i_avg]/3;
					i_nvic=0;
				}
				else if(flag_nvic==2)
				{
			...
			if(flag_nvic==0)
			{
				if(kkk==0)
				{
					if(i_avg>2)
					{
						...
						if(ADC_ConvertedValueNVIC[i_avg-3]-ADC_ConvertedValueNVIC[i_avg-1]>150)
						{
							//BAT1_SW2_OFF;
							i_avg=FRE_NUM;
							kkk=1;
