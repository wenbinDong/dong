...
__IO uint16_t ADC_ConvertedValue[FRE_NUM_DMA];
...
extern __IO uint16_t ADC_ConvertedValue[FRE_NUM_DMA];

// 局部变量，用于保存转换计算后的电压值 	 
__IO uint32_t ADC_ConvertedValueLocal1[FRE_NUM_DMA]; 
__IO uint32_t ADC_ConvertedValueLocal2[FRE_NUM_DMA]; 
__IO uint32_t ADC_ConvertedValueLocal3[FRE_NUM_DMA];
__IO uint32_t ADC_ConvertedValueLocalt[FRE_NUM_DMA];
__IO uint32_t ADC_ConvertedValueLocalt2[FRE_NUM_DMA];
...
/**
  * @brief  配置ADC工作模式
  * @param  DMA_BufferSize = FRE_NUM_DMA
  * @retval 无
  */
static void ADCx_Mode_Config1()
{
  ...
  // DMA_InitStructure.DMA_MemoryBaseAddr = (u32)&ADC_ConvertedValue;
	...
	// 
	DMA_InitStructure.DMA_BufferSize = FRE_NUM_DMA;
...
/**
  ...
  * @retval 无
  */
//L:慢速：buffer:FRE_NUM       6分频13.5采样 DMA传输
...
void ADC_test(uint8_t num,int time)
{
	...
	ADC_SoftwareStartConvCmd(macADCx, ENABLE);
	ADC_DMACmd(macADCx, ENABLE);
	/*这个时间需要根据dma传输的次数有关*///DMA传输时间与FRE_NUM_DMA对应
	Delay_us(time);
...
void BAT1_ADC_U_Two(int U_dis)
{
  ...
  SW2:____________________________|								          |________
	...
	***************************/
	...
	U1 =ADC_ConvertedValue[0];

	BAT1_SW2_OFF;
	//Delay_us(1);
	
	U2 =ADC_ConvertedValue[0];
